...
public class CharacterMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.Space) && !_airTime) && ((Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.A))))
        {
            _animator.Play("jump");
        ...
        if ((Input.GetKeyDown(KeyCode.Space) && !_airTime) && ((Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.D))))
        {
            _animator.Play("jump");
            _rigidBody.AddForce(Vector3.up * _gameStateController.characterJumpFactor, ForceMode.Impulse);
